...
describe("User", () => {
  ...
  it("Should be able to register a user", async () => {
    ...
    const response = await request(app).post("/user").send({
      firstName: "Felipe", lastName: "Molinari", password: "123123", email: "jaajasj@gmail.com",
    })
...
describe('Secures Routes', () => {
  beforeAll(async (done) => {
    ...
    const response = await request(app).post("/login").send({
      email: "jaajasj@gmail.com", password: "123123"
    })
  ...
  it('Should require authorization', async () => {
    const response = await request(app)
      .get('/login/update').send({ gender: "male" })
  ...
  it("Should update user if authenticated", async () => {
    ...
    expect(response.body.newUser.gender).toBe("male")
